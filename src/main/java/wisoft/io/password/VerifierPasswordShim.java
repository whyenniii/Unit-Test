...
public class VerifierPasswordShim {
    private static final Supplier<DayOfWeek> originalDependencies =
            () -> LocalDateTime.now().getDayOfWeek();

    private static Supplier<DayOfWeek> dependencies = originalDependencies;

    public static Runnable inject(Supplier<DayOfWeek> fakes) {
        dependencies = fakes;
        return () -> dependencies = originalDependencies;
    }

    public static List<String> verifyPassword(String input) {
        DayOfWeek dayofweek = dependencies.get();
